   ...
   amr_manager_(amr_manager)
{
  ...
  slam_command_callback_ = [&](uint8_t cmd, bool& result) {
    result = handle_slam_command(cmd);
  };
...
bool CartographerServiceClient::send_and_wait_for_response2(
  VirtualPath::Request::SharedPtr request)
{
  ...
  auto future_result = virtual_path_client_->async_send_request(request, response_received_callback);
  std::unique_lock<std::mutex> lck(mtx_);
  cv_.wait(lck);
